...
const DISABLED_COLOR_UINT8_CLAMPED_ARRAY = makeIconFromColor(
  DISABLED_TAB_COLOR,
  TOOLBAR_ICON_SQUARE_SIZE
).data;
...
q_module.only('background process icon-update tests', () => {
  ...
  q_test('responds to DISABLED content_script_ready appropriately', async (assert) => {
    assert.expect(11);
    function addListener(cb) {
      setTimeout(() => {
        function rcvResponse(arg) {
    ...
    function setIcon({ tabId, imageData }) {
      ...
      assert.deepEqual(
        [...imageData.data],
        [...DISABLED_COLOR_UINT8_CLAMPED_ARRAY],
        'Icon pixels are correct'
      );
    }
    function show(tabId, cb) {
    ...
    function hide(tabId, cb) {
      assert.ok(true, 'chrome.pageAction.hide was called upon receipt of message');
      assert.equal(tabId, -1, 'chrome.pageAction.hide was passed the correct tabId');
      cb();
    }
    setupMessageListeners(
      { addListener },
